...
def init_db():
    ...
    # Injetar dados padrão se estiver totalmente vazio
    cursor.execute("SELECT COUNT(*) FROM collaborators")
    if cursor.fetchone()[0] == 0:
        cursor.execute("INSERT INTO collaborators (id, nome, telefone) VALUES (?, ?, ?)", ("colab-1", "Carlos Silva", "(11) 99999-1111"))
        cursor.execute("INSERT INTO collaborators (id, nome, telefone) VALUES (?, ?, ?)", ("colab-2", "Ana Souza", "(11) 99999-2222"))
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            "client-1", "Empresa XYZ Ltda", "123.456.789-00", "12.345.678/0001-99", "Av. Paulista, 1000 - São Paulo/SP", "contato@xyz.com", "Algar", "colab-1", 1, 1500.0, 350.0, "Roteador Gigabit, Switch de 24 portas, Cabo CAT6.", 1, 1, "Agosto"
        ))
        cursor.execute("""
            INSERT INTO clients (id, nome, cpf, cnpj, endereco, email, empresa, colaborador_id, upfront, valor_upfront, mensalidade, material_instalado, is_contato, is_venda, mes_venda)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            "client-2", "João de Souza", "987.654.321-11", "", "Rua das Flores, 123", "joao@email.com", "Orsegups", "colab-1", 0, 0.0, 0.0, "Nenhum material", 1, 0, ""
        ))
        conn.commit()
    conn.close()
