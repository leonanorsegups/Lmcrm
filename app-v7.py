...
if menu == "Cadastro de Colaboradores":
    ...
    with col1:
        ...
        if st.session_state.edit_colab_id:
            ...
            # Widgets de cadastro de colaborador vinculados à chave dinâmica do formulário
            nome_input = st.text_input("Nome", key=f"new_colab_nome_{st.session_state.colab_form_key}")
            tel_input = st.text_input("Telefone", key=f"new_colab_tel_{st.session_state.colab_form_key}")
            ...
            if st.button("Adicionar Colaborador", type="primary", key="add_colab"):
                if nome_input.strip() == "":
                    ...
                    # Alterar a chave do formulário para resetar os widgets (limpar caixas) de forma limpa e segura
                    st.session_state.colab_form_key = str(uuid.uuid4())
...
# ==========================================
    ...
    with col1:
        if st.session_state.edit_client_id:
            ...
            # Widgets de cadastro de cliente vinculados à chave dinâmica do formulário para permitir limpeza
            c_nome = st.text_input("Nome", key=f"new_c_nome_{st.session_state.client_form_key}")
            c_cpf = st.text_input("CPF", key=f"new_c_cpf_{st.session_state.client_form_key}")
            c_cnpj = st.text_input("CNPJ", key=f"new_c_cnpj_{st.session_state.client_form_key}")
            c_endereco = st.text_input("Endereço", key=f"new_c_endereco_{st.session_state.client_form_key}")
            c_email = st.text_input("Email", key=f"new_c_email_{st.session_state.client_form_key}")
            
            c_empresa = st.selectbox("Empresa", options=["Algar", "Orsegups"], key=f"new_c_empresa_{st.session_state.client_form_key}")
            
            c_colab_name = st.selectbox("Colaborador Associado", options=colab_names, key=f"new_c_colab_{st.session_state.client_form_key}")
            selected_colab_id = colab_options[colab_names.index(c_colab_name)]["id"] if c_colab_name in colab_names else ""
            
            # Classificação Contato x Venda
            st.markdown("##### Classificação do Cliente")
            c_is_contato = st.checkbox("Contato (Prospecção/Atendimento)", value=True, key=f"new_c_is_contato_{st.session_state.client_form_key}")
            c_is_venda = st.checkbox("Venda (Negócio Fechado)", value=False, key=f"new_c_is_venda_{st.session_state.client_form_key}")
            
            c_mes_venda = ""
            if c_is_venda:
                c_mes_venda = st.selectbox("Mês da Venda", options=MESES_ANO, key=f"new_c_mes_venda_{st.session_state.client_form_key}")
            
            st.markdown("---")
            
            c_upfront = st.checkbox("Upfront (Cobrança Inicial)", value=False, key=f"new_c_upfront_{st.session_state.client_form_key}")
            c_valor_upfront = st.number_input(
                "Valor Upfront (R$)", 
                min_value=0.0, 
                value=0.0, 
                step=50.0,
                disabled=not c_upfront,
                key=f"new_c_val_upfront_{st.session_state.client_form_key}"
            )
            ...
            c_mensalidade = st.number_input(
                ...
                value=0.0, 
                step=50.0,
                key=f"new_c_mensalidade_{st.session_state.client_form_key}"
            )
            
            c_material = st.text_area("Material Instalado (Comentários)", key=f"new_c_material_{st.session_state.client_form_key}")
            ...
            if st.button("Adicionar Cliente", type="primary", key="add_client"):
                if c_nome.strip() == "":
                    ...
                    # Alterar a chave de formulário para resetar os widgets (limpar as caixas) de forma limpa e segura
                    st.session_state.client_form_key = str(uuid.uuid4())
