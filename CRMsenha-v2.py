...
def add_user(username, password, nome, telefone=""):
    conn = get_db_connection()
    cursor = conn.cursor()
    pwd_hash = hash_password(password)
    try:
        # Cadastra o usuário de login
...
def update_user(username, password=None, nome=None, telefone=""):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
...
if not st.session_state.autenticado:
    ...
    with col2:
        ...
        st.info("💡 **Acesso padrão do administrador:**\n*   **Usuário:** `admin`\n*   **Senha:** `admin123`\n\n*(Cadastre novos usuários com segurança após o login.)*")
else:
    # Título Principal do Painel
    st.title("💼 Novo CRM - Enterprise Management")
    st.markdown("---")

    # Definir se é administrador logado
    is_admin = (st.session_state.username == "admin")

    # Abas laterais para navegação e operações
    st.sidebar.title("Navegação")
    
    # Montar menu de navegação lateral (Gerenciamento de Usuários apenas visível para o admin)
    ...
    st.sidebar.write(f"Nível de Acesso: **{'Administrador' if is_admin else 'Colaborador'}**")
    ...
    if is_admin:
        ...
        # Colaborador comum só enxerga os próprios dados de perfil
        collaborators_data = [c for c in all_collaborators_raw if c["id"] == st.session_state.username]
        # Colaborador comum só enxerga seus próprios clientes
        ...
        with col_form:
            if st.session_state.edit_colab_id:
                ...
                if colab_to_edit:
                    with st.form("edit_colab_form"):
                        ...
                        if sub_btn:
                            ...
                            st.session_state.edit_colab_id = None
                            st.success("Perfil de colaborador atualizado!")
                            st.rerun()
                ...
                if is_admin:
                    ...
                    st.info("Para manter o sistema consistente, **novos colaboradores devem ser cadastrados criando um usuário de acesso** na aba 'Gerenciamento de Usuários'.")
                else:
        ...
        with col_table:
            ...
            if collaborators_data:
                for colab in collaborators_data:
                    ...
                    col_n.write(f"👤 **{colab['nome']}** (`{colab['id']}`)")
                    col_t.write(colab['telefone'] if colab['telefone'] else "Sem telefone cadastrado")
                    
                    # Permite edição
                    with col_actions:
                        c1, c2 = st.columns(2)
                        if c1.button("📝", key=f"edit_col_{colab['id']}"):
                            st.session_state.edit_colab_id = colab["id"]
                            st.rerun()
                        # Apenas admin pode excluir perfis (exclui o login simultaneamente)
                        if is_admin and colab["id"] != "admin":
                            if c2.button("❌", key=f"del_col_{colab['id']}"):
    ...
    # ==========================================
            ...
            if st.session_state.edit_client_id:
                ...
                if client_to_edit:
                    with st.form("edit_client_form"):
                        ...
                        # Se for admin, pode escolher e reatribuir o colaborador
                        if is_admin:
                            ...
                            selected_colab_id = st.selectbox(
                                ...
                                index=list(colab_options.keys()).index(current_id) if current_id in colab_options else 0
                            )
                        else:
                        ...
                        mensal_cl = st.number_input("Mensalidade (R$)", value=float(client_to_edit["mensalidade"]), min_value=0.0)
                        ...
                        if sub_btn:
                            update_client(
                                ...
                                selected_colab_id, upfront_cl, val_upfront_cl, mensal_cl, mat_instalado,
                                is_contato_cl, is_venda_cl, mes_venda_cl
                            )
                            st.session_state.edit_client_id = None
                ...
                with st.form("add_client_form"):
                    ...
                    mensal_cl = st.number_input("Mensalidade Recorrente (R$)", min_value=0.0)
        ...
        with col_table:
            st.subheader("Clientes Associados ao seu Usuário" if not is_admin else "Todas as Contas do CRM")
            if clients_data:
                for cl in clients_data:
                    ...
                    with col_info:
                        ...
                        st.caption(f"Status: **{status_tag}** | Responsável: **{responsavel_nome}** | E-mail: {cl['email'] if cl['email'] else 'Não informado'}")
                        if cl["is_venda"]:
                            st.caption(f"Faturamento: **R$ {cl['mensalidade']:.2f}/mês** | Upfront: **R$ {cl['valor_upfront']:.2f}** | Mês: **{cl['mes_venda']}**")
                    with col_act:
                        c1, c2 = st.columns(2)
                        if c1.button("📝", key=f"edit_cli_{cl['id']}"):
                            st.session_state.edit_client_id = cl["id"]
                            st.rerun()
                        # Exclusão direta permitida para o admin ou o colaborador dono do registro
                        if c2.button("❌", key=f"del_cli_{cl['id']}"):
                            delete_client(cl["id"])
    ...
    # ==========================================
        ...
        if not is_admin:
            ...
            st.markdown("Como **Administrador**, registre novos membros, altere senhas e gerencie a lista de colaboradores conectados.")
            ...
            with col_form:
                if st.session_state.edit_username:
                    ...
                    cursor = conn.cursor()
                    cursor.execute("""
                        SELECT u.username, u.nome, c.telefone 
                        FROM users u 
                        ...
                        with st.form("edit_user_form"):
                            ...
                            if user_edit_submit:
                                if not edit_name:
                                    ...
                                    success = update_user(st.session_state.edit_username, edit_pwd if edit_pwd else None, edit_name, edit_tel)
                                    if success:
                                        st.success(f"Cadastro de '{st.session_state.edit_username}' atualizado!")
                                        st.session_state.edit_username = None
                                        st.rerun()
                                    else:
                    ...
                    with st.form("add_user_form", clear_on_submit=True):
                        ...
                        if user_submit:
                            ...
                            elif new_username.strip().lower() == "admin" and new_username != "admin":
                                st.error("O identificador 'admin' é restrito do sistema.")
                            else:
                                login_clean = new_username.strip().lower()
                                success = add_user(login_clean, new_password, new_name, new_tel)
                                if success:
                                    st.success(f"Excelente! Usuário '{login_clean}' foi criado e adicionado à lista de colaboradores.")
                                    st.rerun()
                                else:
